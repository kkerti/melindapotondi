// Barion Payment API TypeScript Interfaces
...
export function isBarionError(response: any): response is BarionErrorResponse {
  return response && Array.isArray(response.Errors);
}

export function isPaymentStartResponse(response: any): response is BarionPaymentStartResponse {
  return response && typeof response.PaymentId === 'string' && !isBarionError(response);
}